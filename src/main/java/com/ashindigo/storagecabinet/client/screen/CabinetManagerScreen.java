...
public class CabinetManagerScreen extends ContainerScreen<CabinetManagerContainer> {
    ...
    public CabinetManagerScreen(CabinetManagerContainer container, PlayerInventory playerInv, ITextComponent name) {
    ...
    public void init() {
        ...
        if (TABS.length > 6) {
            addButton(new Button(leftPos, topPos - 50, 20, 20, new StringTextComponent("<"), b -> {
                tabPage = Math.max(tabPage - 1, 0);
                selectTab(TABS[tabPage * 6]);
            }));
            addButton(new Button(leftPos + imageWidth - 20, topPos - 50, 20, 20, new StringTextComponent(">"), b -> {
    ...
    @Override
    public void renderBg(MatrixStack matrixStack, float pPartialTicks, int pX, int pY) {
        ...
        for (int idx = start; idx < end; idx++) {
            ...
            if (tab1 != null && tab1.getId() != selectedTab) {
                textureManager.bind(CREATIVE_INVENTORY_TABS);
                this.renderTabButton(matrixStack, tab1);
            }
        }
        textureManager.bind(BG_TEXTURE);
    ...
    @Override
    public void renderLabels(MatrixStack matrixStack, int pX, int pY) {
        CabinetTab tab = TABS[selectedTab];
        if (tab != null) {
    ...
    private boolean checkTabHovering(MatrixStack matrixStack, CabinetTab tab, int x, int y) {
        int i = tab.getColumn();
        int j = 28 * i;
        int k = 0;
        if (i > 0) {
    ...
    private void renderTabButton(MatrixStack matrixStack, CabinetTab tab) {
        ...
        i1 = i1 - 28;
        // Thanks Forge for the extra fixes
    ...
    public static class CabinetTab {
        ...
        public ITextComponent getTitle() {
            if (entity == null) {
                return new TranslationTextComponent("text.storagecabinet.nocabinets");
            }
