...
public class StorageCabinetRenderer implements BlockEntityRenderer<StorageCabinetEntity> {
    ...
    private void drawKey(boolean locked, MatrixStack matrices, VertexConsumerProvider provider, int overlay) {
        if (locked) {
            MinecraftClient.getInstance().getItemRenderer().renderItem(KEY, ModelTransformation.Mode.GUI, 0x00f000f0, overlay, matrices, provider, 0);
        }
    }
    ...
    private void drawStack(ItemStack stack, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int overlay) {
        ...
        MinecraftClient.getInstance().getItemRenderer().renderItem(stack, ModelTransformation.Mode.GUI, 0x00f000f0, overlay, matrices, vertexConsumers, 0);
        if (stack.getItem() instanceof BlockItem) {
