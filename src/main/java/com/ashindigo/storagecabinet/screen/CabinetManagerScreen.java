...
public class CabinetManagerScreen extends BaseContainerScreen<CabinetManagerContainer> {
    ...
    public CabinetManagerScreen(CabinetManagerContainer linkedContainer, PlayerInventory playerInv, Text name) {
        ...
        final int[] i = {1};
        checkSurroundingCabinets(cabinetList, linkedContainer.managerEntity.getPos(), getContainer().getWorld());
    ...
    private void checkSurroundingCabinets(ArrayList<StorageCabinetEntity> cabinetList, BlockPos pos, World world) { // TODO Probably do max range, and dear god that performance.
    ...
    private void addCabinet(WTabHolder tabHolder, StorageCabinetEntity cabinetEntity, int invNumb) {
        ...
        WVerticalScrollableContainer panel = tab.getBody().createChild(WVerticalScrollableContainer::new, Position.ofBottomLeft(tabHolder).add(4, -(162 + (5 * 18) + 8), 0), Size.of((10 * 18) + 18, 162));
        ...
        for (int y = 0; y < StorageCabinetBlock.Manager.getHeight(cabinetEntity.tier); ++y) {
            ...
            panel.addRow(slotArr); // Ugh this is probably a hard hitter for performance
        }
    }
}
